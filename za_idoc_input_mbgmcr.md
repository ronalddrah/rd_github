...
* local data
  ...
  DATA: BEGIN OF it_lock OCCURS 0,
          ...
          lock  TYPE c,
        END OF it_lock,
        it_split_items      TYPE STANDARD TABLE OF bapi2017_gm_item_create,
        ls_split_item       TYPE bapi2017_gm_item_create,
...
* RBDC
  ...
  DATA: gt_mara_buf      TYPE HASHED TABLE OF mara WITH UNIQUE KEY matnr,
        ...
        gt_ekpo_buf      TYPE HASHED TABLE OF ekpo WITH UNIQUE KEY ebeln ebelp
                         WITH NON-UNIQUE SORTED KEY k_bednr COMPONENTS bednr afnam
        ...
        gt_ekbe_buf      TYPE SORTED TABLE OF ekbe WITH NON-UNIQUE KEY ebeln ebelp
                         WITH NON-UNIQUE SORTED KEY k_ref   COMPONENTS lfbnr lfpos lfgja shkzg vgabe,
        ...
        gt_t156_buf      TYPE HASHED TABLE OF ty_t156 WITH UNIQUE KEY bwart,
        ...
        gt_mbgmcr_lib    TYPE HASHED TABLE OF yusmm_mbgmcr_lib WITH UNIQUE KEY sndprn,
        ...
        gt_tvshp_buf     TYPE STANDARD TABLE OF tvshp,
        gt_mbgmcr_tr     TYPE SORTED TABLE OF y0mm_mbgmcr_tr WITH NON-UNIQUE KEY sndprn transfer,
        gt_braziltr_buf  TYPE SORTED TABLE OF ybrmm_idoc_tr WITH NON-UNIQUE KEY mestyp werks lgort,
        gt_fill_plant    TYPE HASHED TABLE OF y0pp_fill_plant WITH UNIQUE KEY werks_prod,
        gt_mbew_buf      TYPE HASHED TABLE OF mbew WITH UNIQUE KEY matnr bwkey,
        lt_inb_lips      TYPE STANDARD TABLE OF ty_inb_lips,
        lt_t001          TYPE STANDARD TABLE OF t001l,
        ls_yudc_prefetch TYPE yudc_trans.

  " Collect keys for pre-fetch
  DATA: lt_matnr_keys TYPE SORTED TABLE OF matnr WITH UNIQUE KEY table_line,
        ...
        lt_bednr_keys TYPE SORTED TABLE OF ebeln WITH UNIQUE KEY table_line.
...
* get customer customizing
  ...
  " Collect all relevant keys from all IDocs
  LOOP AT idoc_data INTO DATA(ls_data_prefetch).
    CASE ls_data_prefetch-segnam.
      WHEN co_mbgmcr_item.
        ls_mbgmcr_item = ls_data_prefetch-sdata.
        ...
        IF ls_mbgmcr_item-po_number IS NOT INITIAL.
          ...
          INSERT ls_mbgmcr_item-po_number INTO TABLE lt_bednr_keys.
        ENDIF.
        ...
        IF ls_mbgmcr_item-vendor IS NOT INITIAL.
          DATA(lv_vendor_prefetch) = ls_mbgmcr_item-vendor.
          CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
            EXPORTING
              input  = lv_vendor_prefetch
            IMPORTING
              output = lv_vendor_prefetch.
          INSERT lv_vendor_prefetch INTO TABLE lt_lifnr_keys.
        ENDIF.
    ENDCASE.
  ENDLOOP.
  ...
  IF lt_matnr_keys IS NOT INITIAL.
    SELECT * FROM mara INTO TABLE @gt_mara_buf FOR ALL ENTRIES IN @lt_matnr_keys WHERE matnr = @lt_matnr_keys-table_line.
    SELECT * FROM y0pp_calc_rbfqty INTO TABLE @gt_rbfqty_buf FOR ALL ENTRIES IN @lt_matnr_keys WHERE matnr = @lt_matnr_keys-table_line.
  ENDIF.

  IF lt_ebeln_keys IS NOT INITIAL.
    SELECT * FROM ekko INTO TABLE @gt_ekko_buf FOR ALL ENTRIES IN @lt_ebeln_keys WHERE ebeln = @lt_ebeln_keys-table_line.
    SELECT * FROM ekpo INTO TABLE @DATA(lt_ekpo_prefetch) FOR ALL ENTRIES IN @lt_ebeln_keys WHERE ebeln = @lt_ebeln_keys-table_line.
    IF lt_bednr_keys IS NOT INITIAL.
      SELECT * FROM ekpo APPENDING TABLE @lt_ekpo_prefetch FOR ALL ENTRIES IN @lt_bednr_keys WHERE bednr = @lt_bednr_keys-table_line.
    ENDIF.
    SORT lt_ekpo_prefetch BY ebeln ebelp.
    DELETE ADJACENT DUPLICATES FROM lt_ekpo_prefetch COMPARING ebeln ebelp.
    gt_ekpo_buf = lt_ekpo_prefetch.

    SELECT * FROM eket INTO TABLE @gt_eket_buf FOR ALL ENTRIES IN @lt_ebeln_keys WHERE ebeln = @lt_ebeln_keys-table_line.
    SELECT * FROM ekes INTO TABLE @gt_ekes_buf FOR ALL ENTRIES IN @lt_ebeln_keys WHERE ebeln = @lt_ebeln_keys-table_line.
    SELECT * FROM ekbe INTO TABLE @gt_ekbe_buf FOR ALL ENTRIES IN @lt_ebeln_keys WHERE ebeln = @lt_ebeln_keys-table_line.

    " Collect related inbound deliveries from confirmations (EKES) and LIPS
    LOOP AT gt_ekes_buf INTO DATA(ls_ekes_tmp).
       IF ls_ekes_tmp-vbeln IS NOT INITIAL. INSERT ls_ekes_tmp-vbeln INTO TABLE lt_vbeln_keys. ENDIF.
    ENDLOOP.
    SELECT vbeln FROM lips FOR ALL ENTRIES IN @lt_ebeln_keys WHERE vgbel = @lt_ebeln_keys-table_line INTO TABLE @DATA(lt_vbeln_tmp).
    LOOP AT lt_vbeln_tmp INTO DATA(ls_vbeln_tmp).
       INSERT ls_vbeln_tmp-vbeln INTO TABLE lt_vbeln_keys.
    ENDLOOP.
  ENDIF.

  IF lt_vbeln_keys IS NOT INITIAL.
    SELECT * FROM likp INTO TABLE @gt_likp_buf FOR ALL ENTRIES IN @lt_vbeln_keys WHERE vbeln = @lt_vbeln_keys-table_line.
    SELECT * FROM lips INTO TABLE @gt_lips_buf FOR ALL ENTRIES IN @lt_vbeln_keys WHERE vbeln = @lt_vbeln_keys-table_line.
  ENDIF.

  IF lt_werks_keys IS NOT INITIAL.
    SELECT * FROM t001w INTO TABLE @gt_t001w_buf FOR ALL ENTRIES IN @lt_werks_keys WHERE werks = @lt_werks_keys-table_line.
    SELECT * FROM t001l INTO TABLE @gt_t001l_buf FOR ALL ENTRIES IN @lt_werks_keys WHERE werks = @lt_werks_keys-table_line.
    SELECT bwkey FROM t001w FOR ALL ENTRIES IN @lt_werks_keys WHERE werks = @lt_werks_keys-table_line INTO TABLE @lt_bwkey_keys.
    IF lt_bwkey_keys IS NOT INITIAL.
      SELECT * FROM t001k INTO TABLE @gt_t001k_buf FOR ALL ENTRIES IN @lt_bwkey_keys WHERE bwkey = @lt_bwkey_keys-table_line.
      IF lt_matnr_keys IS NOT INITIAL.
        SELECT * FROM mbew INTO TABLE @gt_mbew_buf FOR ALL ENTRIES IN @lt_matnr_keys WHERE matnr = @lt_matnr_keys-table_line.
      ENDIF.
    ENDIF.
  ENDIF.

  " Configuration and lookup tables
  SELECT bwart, shkzg FROM t156 INTO TABLE @DATA(lt_t156_prefetch).
  LOOP AT lt_t156_prefetch INTO DATA(ls_t156_prefetch).
    INSERT VALUE #( bwart = ls_t156_prefetch-bwart shkzg = ls_t156_prefetch-shkzg ) INTO TABLE gt_t156_buf.
  ENDLOOP.
  ...
  SELECT * FROM yusmm_mbgmcr_lib INTO TABLE @gt_mbgmcr_lib.
  SELECT * FROM y0ca_ale_delay INTO TABLE @gt_ale_delay_buf.
  SELECT * FROM yudc_trans INTO TABLE @gt_yudc_trans.
  SELECT * FROM tvshp INTO TABLE @gt_tvshp_buf.
  SELECT * FROM y0mm_mbgmcr_tr INTO TABLE @gt_mbgmcr_tr.
  SELECT * FROM ybrmm_idoc_tr INTO TABLE @gt_braziltr_buf.
  SELECT * FROM y0pp_fill_plant INTO TABLE @gt_fill_plant.

  IF lt_matnr_keys IS NOT INITIAL.
     SELECT * FROM qals INTO TABLE @gt_qals_buf FOR ALL ENTRIES IN @lt_matnr_keys WHERE matnr = @lt_matnr_keys-table_line.
  ENDIF.
...
*   select segments belonging to one IDoc                              *
    ...
    IF sy-subrc = 0.
      DATA(lv_idoc_data_index) = sy-tabix.
      LOOP AT idoc_data FROM lv_idoc_data_index INTO DATA(ls_idoc_data_sel).
        IF ls_idoc_data_sel-docnum <> idoc_contrl-docnum.
          EXIT.
        ENDIF.
        APPEND ls_idoc_data_sel TO t_edidd.
      ENDLOOP.
    ENDIF.
...
*   unlock previos po's
    IF it_lock[] IS NOT INITIAL.
      DATA: lt_ebeln_unlock TYPE TABLE OF y0mm_proc_ebeln.
      LOOP AT it_lock.
        APPEND VALUE #( ebeln = it_lock-ebeln ) TO lt_ebeln_unlock.
      ENDLOOP.
      DELETE y0mm_proc_ebeln FROM TABLE lt_ebeln_unlock.
      COMMIT WORK.
    ENDIF.
*   through all segments of this IDoc                                  *
    ...
    CATCH SYSTEM-EXCEPTIONS conversion_errors = 1.
      ...
      ENDIF.
      DATA(ls_dd_plt) = VALUE #( gt_dd_plt_buf[ partyp = idoc_contrl-sndprt
                                                parnum = idoc_contrl-sndprn
                                                mestyp = idoc_contrl-mestyp ] OPTIONAL ).
      IF ls_dd_plt IS NOT INITIAL.
      ...
      IF ls_likp_bi IS NOT INITIAL AND ls_likp_bi-vbtyp NE '7'.
...
*         recalculate quantites
            DATA(ls_rbfqty) = VALUE #( gt_rbfqty_buf[ matnr = goodsmvt_item-material ] OPTIONAL ).
...
*         In case of YIDV delivery - get PO number if not included in Idoc
            IF goodsmvt_item-po_number IS INITIAL.
              IF idoc_contrl-sndprn = 'ATCPSYS' OR idoc_contrl-sndprn = 'DEPVSFASH'.
                DATA(ls_lips_prefetch) = VALUE #( gt_lips_buf[ vbeln = goodsmvt_item-deliv_numb_to_search posnr = goodsmvt_item-deliv_item_to_search ] OPTIONAL ).
                IF ls_lips_prefetch IS NOT INITIAL.
                   goodsmvt_item-po_number = ls_lips_prefetch-vgbel.
                   goodsmvt_item-po_item   = ls_lips_prefetch-vgpos.
                ENDIF.
              ELSE.
                DATA(ls_likp_prefetch) = VALUE #( gt_likp_buf[ vbeln = goodsmvt_item-deliv_numb_to_search ] OPTIONAL ).
                IF ls_likp_prefetch IS NOT INITIAL AND ls_likp_prefetch-lfart = co_lfart_yidv.
                  ls_lips_prefetch = VALUE #( gt_lips_buf[ vbeln = goodsmvt_item-deliv_numb_to_search posnr = goodsmvt_item-deliv_item_to_search ] OPTIONAL ).
                  IF ls_lips_prefetch IS NOT INITIAL.
                     goodsmvt_item-po_number = ls_lips_prefetch-vgbel.
                     goodsmvt_item-po_item   = ls_lips_prefetch-vgpos.
                  ENDIF.
            ...
            IF ls_ekpo_weora IS NOT INITIAL AND ls_ekpo_weora-weora = 'X'.
               hi_weora = 'X'.
            ENDIF.
            ...
            IF hi_weora = 'X'.
              IF goodsmvt_item-move_type EQ '101'.
                IF goodsmvt_item-deliv_numb_to_search IS INITIAL.
                  LOOP AT gt_ekes_buf INTO DATA(ls_ekes_prefetch) WHERE ebeln = goodsmvt_item-po_number
                ...
                IF goodsmvt_item-deliv_numb_to_search IS NOT INITIAL.
                  IF NOT idoc_contrl-sndprn = 'ATCPSYS' AND NOT idoc_contrl-sndprn = 'DEPVSFASH'.
                    IF NOT line_exists( gt_tvshp_buf[ spe_inb_vl_mm = abap_true ] ).
                      CLEAR: goodsmvt_item-po_number, goodsmvt_item-po_item.
                    ENDIF.
                  ENDIF.
                  goodsmvt_item-move_type = '109'.
                ELSE.
                  LOOP AT gt_ekes_buf INTO ls_ekes_prefetch WHERE ebeln = goodsmvt_item-po_number
                                                              AND ebelp = goodsmvt_item-po_item
                                                              AND charg = goodsmvt_item-batch
                                                              AND vbeln NE space.
                    goodsmvt_item-deliv_numb_to_search = ls_ekes_prefetch-vbeln.
...
*           Step 1: check doc.type of PO, Hard coded ZRM
            IF line_exists( gt_ekko_buf[ ebeln = goodsmvt_item-po_number ] ) AND
               gt_ekko_buf[ ebeln = goodsmvt_item-po_number ]-bsart = 'ZRM'.
*              Step 2 & 3: Get ZRU PO Items via Tracking Number and Search with requisitioner
               LOOP AT gt_ekpo_buf INTO DATA(ls_ekpo_prefetch) USING KEY k_bednr
                                                               WHERE bednr = goodsmvt_item-po_number
                                                                 AND afnam = goodsmvt_item-po_item.
                  goodsmvt_item-po_number = ls_ekpo_prefetch-ebeln.
                  goodsmvt_item-po_item   = ls_ekpo_prefetch-ebelp.
                  EXIT.
               ENDLOOP.
            ENDIF.
...
* POS inbound delivery
            IF NOT goodsmvt_item-deliv_numb_to_search IS INITIAL.
              ...
              DATA(lv_lfart_prefetch) = VALUE #( gt_likp_buf[ vbeln = goodsmvt_item-deliv_numb_to_search ]-lfart OPTIONAL ).
              READ TABLE it_y0mm_inbounddeli
                          WITH KEY zndprn  = idoc_contrl-sndprn
                                    lfart  = lv_lfart_prefetch
                                    bwart  = goodsmvt_item-move_type
                                    mtart  = gf_mtart.
              IF sy-subrc EQ 0.
...
* fields, therefor table EKBE (history of purchase doc.) must be read
            IF gf_inbound_del_flag NE 'X'.
              IF line_exists( gt_t156_buf[ bwart = goodsmvt_item-move_type ] ) AND
                 gt_t156_buf[ bwart = goodsmvt_item-move_type ]-shkzg EQ 'H'.
                ...
                LOOP AT gt_ekbe_buf INTO wa_ekbe WHERE ebeln = goodsmvt_item-po_number
                   ...
                   READ TABLE gt_ekbe_buf TRANSPORTING NO FIELDS USING KEY k_ref
                ...
                IF goodsmvt_item-ref_doc IS INITIAL.
                  LOOP AT gt_ekbe_buf INTO wa_ekbe WHERE ebeln = goodsmvt_item-po_number
                                                    ...
                                                    AND vgabe = '1'.
                    CLEAR hi_menge.
                    LOOP AT gt_ekbe_buf INTO DATA(ls_ekbe_cancel) USING KEY k_ref
                                                                    WHERE lfbnr = wa_ekbe-lfbnr
                                                                      AND lfpos = wa_ekbe-lfpos
                                                                      AND lfgja = wa_ekbe-lfgja
                                                                      AND shkzg = 'H'
                                                                      AND vgabe = '1'.
                      IF ls_ekbe_cancel-ebeln = goodsmvt_item-po_number AND
                         ls_ekbe_cancel-ebelp = goodsmvt_item-po_item   AND
                         ls_ekbe_cancel-matnr = goodsmvt_item-material  AND
                         ls_ekbe_cancel-werks = goodsmvt_item-plant.
...
* Reversal will be created manually
              ...
              IF ls_mara_label IS NOT INITIAL AND ls_mara_label-labor EQ co_labor_etikett.
                IF goodsmvt_code = '01'.
                  IF t156-shkzg EQ 'S'.
...
* Save order quantity and item
                    ...
                    LOOP AT gt_ekpo_buf INTO ls_ekpo_prefetch USING KEY k_mat
                                                               WHERE werks = goodsmvt_item-plant
                                                                 AND matnr = goodsmvt_item-material.
                       CHECK ls_ekpo_prefetch-loekz = space AND ls_ekpo_prefetch-elikz = space AND ls_ekpo_prefetch-bstyp = 'F'.
                       IF line_exists( gt_ekko_buf[ ebeln = ls_ekpo_prefetch-ebeln ] ) AND
                          gt_ekko_buf[ ebeln = ls_ekpo_prefetch-ebeln ]-lifnr = goodsmvt_item-vendor.
                          APPEND VALUE #( ebeln = ls_ekpo_prefetch-ebeln
                                          ebelp = ls_ekpo_prefetch-ebelp
                                          bedat = gt_ekko_buf[ ebeln = ls_ekpo_prefetch-ebeln ]-bedat ) TO it_v_ekko_ekpo.
                       ENDIF.
                    ENDLOOP.
                    ...
                    IF it_v_ekko_ekpo[] IS NOT INITIAL.
                      ...
                      LOOP AT it_v_ekko_ekpo.
                         LOOP AT gt_eket_buf INTO DATA(ls_eket_prefetch) WHERE ebeln = it_v_ekko_ekpo-ebeln
                                                                           AND ebelp = it_v_ekko_ekpo-ebelp.
                            APPEND ls_eket_prefetch TO it_eket.
                         ENDLOOP.
                      ENDLOOP.
...
* Get open quantitiy
                        LOOP AT it_eket.
                          REFRESH it_ekes.
                          LOOP AT gt_ekes_buf INTO ls_ekes_prefetch WHERE ebeln = it_eket-ebeln
                                                                      AND ebelp = it_eket-ebelp
                                                                      AND loekz = space.
                             APPEND ls_ekes_prefetch TO it_ekes.
                          ENDLOOP.
...
*--------------------------------------------------------
* If quantity is enough, recalculate split items and post once
                    IF hi_entry_qnt EQ 0.
                      hi_flag_etikett = 'X'.             "Mark for label
* Sort by date
                      SORT it_eket BY bedat ebeln ebelp.
                      hi_entry_qnt  = goodsmvt_item-entry_qnt.
                      REFRESH it_split_items.
...
* Calculate splits
                      LOOP AT it_eket.
                        ...
                        IF it_ekes[] IS NOT INITIAL.
                          LOOP AT it_ekes.
                            ...
                            CLEAR ls_split_item.
                            ls_split_item = goodsmvt_item_save.

                            IF hi_omeng LT hi_entry_qnt.
                              ls_split_item-entry_qnt = hi_omeng.
                              ls_split_item-no_more_gr = 'X'.
                              hi_entry_qnt = hi_entry_qnt - hi_omeng.
                            ELSE.
                              ls_split_item-entry_qnt = hi_entry_qnt.
                              ls_split_item-no_more_gr = space.
                              hi_entry_qnt = 0.
                            ENDIF.

                            CONCATENATE goodsmvt_item_save-po_number ' ' goodsmvt_item_save-po_item INTO ls_split_item-unload_pt.
                            ls_split_item-po_number = it_ekes-ebeln.
                            ls_split_item-po_item   = it_ekes-ebelp.

                            APPEND ls_split_item TO it_split_items.
                      ...
                      IF it_split_items[] IS NOT INITIAL.
* Post consolidated goods movement
                        CALL FUNCTION 'BAPI_GOODSMVT_CREATE'
                          ...
                          TABLES
                            goodsmvt_item         = it_split_items
                            goodsmvt_serialnumber = goodsmvt_serialnumber
                            return                = return
                          EXCEPTIONS
                            OTHERS                = 1.

                        IF sy-subrc <> 0 OR line_exists( return[ type = 'E' ] ) OR line_exists( return[ type = 'A' ] ).
                          error_flag = 'X'.
                          " Error handling as before...
                          IF sy-subrc <> 0.
                             PERFORM insert_status USING '51' 'E' sy-msgid sy-msgno sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
                          ELSE.
                             LOOP AT return INTO DATA(ls_ret_split) WHERE type = 'E' OR type = 'A'.
                                PERFORM insert_status USING '51' ls_ret_split-type ls_ret_split-id ls_ret_split-number ls_ret_split-message_v1 ls_ret_split-message_v2 ls_ret_split-message_v3 ls_ret_split-message_v4.
                             ENDLOOP.
                          ENDIF.
                        ELSE.
                          COMMIT WORK AND WAIT.
                          PERFORM insert_status USING '53' 'S' 'B1' '501' 'CREATEFROMDATA' space space space.
...
*                  In case of reversal get corresponding 101 GR
                  IF line_exists( gt_lips_buf[ vbeln = goodsmvt_item-deliv_numb_to_search posnr = goodsmvt_item-deliv_item_to_search ] ).
                     ls_lips_prefetch = gt_lips_buf[ vbeln = goodsmvt_item-deliv_numb_to_search posnr = goodsmvt_item-deliv_item_to_search ].
                     goodsmvt_item-po_number = ls_lips_prefetch-vgbel.
                     goodsmvt_item-po_item   = ls_lips_prefetch-vgpos.
                  ENDIF.
                ...
                ELSE.
                  ...
                  IF line_exists( gt_mbgmcr_lib[ sndprn = idoc_contrl-sndprn ] ).
                    LOOP AT gt_ekbe_buf INTO wa_ekbe WHERE ebeln = goodsmvt_item-po_number
                                                      AND ebelp = goodsmvt_item-po_item
                                                      AND vgabe = '8'
                                                      AND menge = 0.
...
* If there is one, and the actual movement type is 101 - change to 109
      ...
      IF line_exists( gt_weora_buf[ rcvpor = idoc_contrl-sndprn ] ).
...
*       Get possible reverse documents
            REFRESH it_ekbe_rev.
            LOOP AT gt_ekbe_buf INTO DATA(ls_ekbe_buf) WHERE ebeln = goodsmvt_item-po_number
                                                        AND ebelp = goodsmvt_item-po_item
                                                        AND bwart = '109'.
               MOVE-CORRESPONDING ls_ekbe_buf TO it_ekbe_rev.
               COLLECT it_ekbe_rev.
            ENDLOOP.

*            Get possible reverse docs from 109
            REFRESH it_ekbe_rev_cancel.
            LOOP AT gt_ekbe_buf INTO ls_ekbe_buf WHERE ebeln = goodsmvt_item-po_number
                                                   AND ebelp = goodsmvt_item-po_item
                                                   AND ( bwart = '110' OR bwart = '108' ).
               MOVE-CORRESPONDING ls_ekbe_buf TO it_ekbe_rev_cancel.
                ...
                LOOP AT goodsmvt_item.
                  ...
                  LOOP AT it_ekbe_rev WHERE ebeln = goodsmvt_item-po_number
                    ...
                    IF goodsmvt_item-stck_type IS INITIAL.
                       ...
                       IF ls_ekpo_stck IS NOT INITIAL.
...
*   get delay values
      ...
      IF line_exists( gt_ale_delay_buf[ mesty = idoc_contrl-mestyp ] ).
         y0ca_ale_delay = gt_ale_delay_buf[ mesty = idoc_contrl-mestyp ].
      ENDIF.
      ...
      WHILE all_locked = space AND y0ca_ale_delay-retry > 0.
        ...
        COMMIT WORK. " Single commit after bulk insert
...
* set usage decision
      ...
      IF l_lines > 0.
        ...
        "S4H-1922: GR via inbound delivery update if packing relevant
        IF goodsmvt_code = '01' AND NOT line_exists( goodsmvt_item[ move_type = '107' ] ) AND /spe/cl_cust=>is_vl_mm_active( ) = abap_true.
          REFRESH lt_inb_lips.
          LOOP AT goodsmvt_item INTO ls_migo_itm.
            IF ls_migo_itm-deliv_numb_to_search IS NOT INITIAL.
               "Direct inbound delivery reference
               LOOP AT gt_lips_buf INTO DATA(ls_lips_prefetch_hu) WHERE vbeln = ls_migo_itm-deliv_numb_to_search.
                  CHECK ls_lips_prefetch_hu-posnr = ls_migo_itm-deliv_item_to_search OR ls_lips_prefetch_hu-uecha = ls_migo_itm-deliv_item_to_search.
                  IF line_exists( gt_likp_buf[ vbeln = ls_lips_prefetch_hu-vbeln ] ) AND
                     gt_likp_buf[ vbeln = ls_lips_prefetch_hu-vbeln ]-vbtyp = '7' AND
                     gt_likp_buf[ vbeln = ls_lips_prefetch_hu-vbeln ]-pkstk <> ''.
                     APPEND CORRESPONDING #( ls_lips_prefetch_hu ) TO lt_inb_lips.
                  ENDIF.
               ENDLOOP.
            ELSEIF ls_migo_itm-po_number IS NOT INITIAL.
               "PO reference -> find packing relevant inbound deliveries
               LOOP AT gt_lips_buf INTO ls_lips_prefetch_hu USING KEY k_ref
                                                             WHERE vgbel = ls_migo_itm-po_number
                                                               AND vgpos = |{ ls_migo_itm-po_item ALPHA = IN }|.
                  IF line_exists( gt_likp_buf[ vbeln = ls_lips_prefetch_hu-vbeln ] ) AND
                     gt_likp_buf[ vbeln = ls_lips_prefetch_hu-vbeln ]-vbtyp = '7' AND
                     gt_likp_buf[ vbeln = ls_lips_prefetch_hu-vbeln ]-pkstk <> ''.
                     APPEND CORRESPONDING #( ls_lips_prefetch_hu ) TO lt_inb_lips.
        ...
        IF lt_inb_lips IS NOT INITIAL.
          ...
          LOOP AT lt_inb_lips INTO DATA(ls_inb_lips_hu).
             IF line_exists( gt_t001l_buf[ werks = ls_inb_lips_hu-werks lgort = ls_inb_lips_hu-lgort ] ) AND
                gt_t001l_buf[ werks = ls_inb_lips_hu-werks lgort = ls_inb_lips_hu-lgort ]-xhupf = 'X'.
                APPEND VALUE #( werks = ls_inb_lips_hu-werks lgort = ls_inb_lips_hu-lgort xhupf = 'X' ) TO lt_t001.
             ENDIF.
          ENDLOOP.
          ...
          LOOP AT lt_t001 INTO DATA(ls_t001).
            LOOP AT lt_inb_lips INTO ls_inb_lips WHERE werks = ls_t001-werks AND lgort = ls_t001-lgort.
              APPEND VALUE #( vbeln = |{ ls_inb_lips-vbeln ALPHA = IN }| ) TO lt_deliv.
            ENDLOOP.
          ENDLOOP.
          SORT lt_deliv.
        ...
        IF lt_inb_lips IS NOT INITIAL.
          ...
          DELETE lt_inb_lips WHERE NOT wbsta CA 'AB'. "Correct status check
...
*         RBDC - Save document/IDOC information
        IF idoc_contrl-mescod EQ 'RDC'.
          ...
          READ TABLE gt_yudc_trans INTO ls_yudc_prefetch WITH KEY bukrs = l_bukrs
                                                                  gm_code = goodsmvt_code
                                                                  lgort_from = goodsmvt_item-stge_loc
                                                                  bwart = goodsmvt_item-move_type
                                                                  lgort_to = goodsmvt_item-move_stloc.
          IF sy-subrc = 0.
             wa_run_i-zztrans_id = ls_yudc_prefetch-zztrans_id.
          ENDIF.
...
*         RBDC - Save document/IDOC information
          IF idoc_contrl-mescod EQ 'RDC'.
            READ TABLE goodsmvt_item INDEX 1.
            DATA(lv_bwkey_rbdc) = VALUE #( gt_t001w_buf[ werks = goodsmvt_item-plant ]-bwkey OPTIONAL ).
            DATA(lv_bukrs_rbdc) = VALUE #( gt_t001k_buf[ bwkey = lv_bwkey_rbdc ]-bukrs OPTIONAL ).
            READ TABLE gt_yudc_trans INTO ls_yudc_prefetch WITH KEY bukrs = lv_bukrs_rbdc
                                                                    gm_code = goodsmvt_code
                                                                    lgort_from = goodsmvt_item-stge_loc
                                                                    bwart = goodsmvt_item-move_type
                                                                    lgort_to = goodsmvt_item-move_stloc.
...
*         RBDC - Save document/IDOC information
          IF idoc_contrl-mescod EQ 'RDC'.
            READ TABLE goodsmvt_item INDEX 1.
            lv_bwkey_rbdc = VALUE #( gt_t001w_buf[ werks = goodsmvt_item-plant ]-bwkey OPTIONAL ).
            lv_bukrs_rbdc = VALUE #( gt_t001k_buf[ bwkey = lv_bwkey_rbdc ]-bukrs OPTIONAL ).
            READ TABLE gt_yudc_trans INTO ls_yudc_prefetch WITH KEY bukrs = lv_bukrs_rbdc
                                                                    gm_code = goodsmvt_code
                                                                    lgort_from = goodsmvt_item-stge_loc
                                                                    bwart = goodsmvt_item-move_type
...
* get customizing settings
  REFRESH it_braziltr.
  LOOP AT it_lips.
     LOOP AT gt_braziltr_buf INTO DATA(ls_tr_prefetch) WHERE mestyp = idoc_contrl-mestyp
                                                         AND werks  = it_lips-werks
                                                         AND lgort  = it_lips-lgort.
        APPEND CORRESPONDING #( ls_tr_prefetch ) TO it_braziltr.
     ENDLOOP.
  ENDLOOP.
...
*      get data
      ...
      DATA(ls_mbew_prefetch) = VALUE #( gt_mbew_buf[ matnr = it_lips-matnr bwkey = it_lips-werks ] OPTIONAL ).
      IF ls_mbew_prefetch IS NOT INITIAL.
        lk_peinh = ls_mbew_prefetch-peinh.
        lk_bwprs = ls_mbew_prefetch-bwprs.
...
FORM transfer_posting USING p_pre_posting TYPE flag.
  DATA: lv_mat_doc        TYPE bapi2017_gm_head_ret-mat_doc,
        ...
        lt_mbgmcr_tr_local TYPE TABLE OF y0mm_mbgmcr_tr.
  ...
  "check cust.
  REFRESH lt_mbgmcr_tr_local.
  LOOP AT gt_mbgmcr_tr INTO DATA(ls_mbgmcr_tr_local) WHERE sndprn = idoc_contrl-sndprn
                                                       AND transfer = p_pre_posting.
     APPEND ls_mbgmcr_tr_local TO lt_mbgmcr_tr_local.
  ENDLOOP.
  CHECK lt_mbgmcr_tr_local[] IS NOT INITIAL.
  ...
  LOOP AT goodsmvt_item ASSIGNING FIELD-SYMBOL(<fs_goodsmvt_item>).
    "relevant?
    READ TABLE lt_mbgmcr_tr_local INTO DATA(ls_mbgmcr_tr_cur) WITH KEY move_type = <fs_goodsmvt_item>-move_type
    ...
    ls_gm_item_create-move_type = ls_mbgmcr_tr_cur-move_type_tr.
